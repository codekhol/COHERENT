...
#endif
{
  ...
  /* dr_info is defined with 255 words in hdioctl.h */
  /* this is why we do not use magic numbers - Udo */
  repinsw(DATA_REG, (ushort_t *)dr_info, sizeof(ide_info_t) / 2);
...
#endif
{
  ...
  for (u = 0, dp = atparm; u < at_drive_ct; ++ dp, ++ u) {
    ...
    if (at_get_drive_id(u) == 0) {
      ti = &ide_drive_info[u];
      /* we do not use magic numbers if we don't have to - Udo */
      /*at_byte_order_copy(tmp_model, (unsigned char *)ti->ii_modelnum, 40);*/
      at_byte_order_copy(tmp_model, (unsigned char *)ti->ii_modelnum,
			 sizeof(ti->ii_modelnum));
      /*at_byte_order_copy(tmp_rev, (unsigned char *)ti->ii_firmrev, 8);*/
      at_byte_order_copy(tmp_rev, (unsigned char *)ti->ii_firmrev,
			 sizeof(ti->ii_firmrev));
      /*at_byte_order_copy(tmp_serno, (unsigned char *)ti->ii_serialnum, 20);*/
      at_byte_order_copy(tmp_serno, (unsigned char *)ti->ii_serialnum,
			 sizeof(ti->ii_serialnum));
...
/*
 * void
 * atread (dev, iop)	- read a block from the raw disk
 * dev_t dev;
 * IO * iop;
 *
 *	Input:	dev = disk device to be read from.
